...
export function Employees() {
  ...
  const filteredEmployees = employees?.filter(employee => {
    const matchesSearch =
      employee.roleSlug.toLowerCase().includes(searchQuery.toLowerCase()) ||
  ...
  const handleRemoveEmployee = async () => {
    ...
    try {
      await apiRequest<void>(`/api/employees/${employeeToRemove.id}`, {
        method: 'DELETE',
      });
  ...
  const runningCount = employees?.filter(c => c.status === 'running').length || 0;
  const stoppedCount = employees?.filter(c => c.status === 'stopped').length || 0;
  const errorCount = employees?.filter(c => c.status === 'error').length || 0;
                ...
                filteredEmployees?.map((employee) => (
                  ...
                  >
                    <td className="py-4 px-6">
                      <span className="font-medium text-cyber-cyan hover:text-cyber-white">{employee.roleSlug}</span>
                    </td>
                    <td className="py-4 px-6">
                    ...
                    <td className="py-4 px-6">
                      <span className="font-mono text-cyber-cyan">{employee.port}</span>
                    </td>
                    <td className="py-4 px-6">
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/70 backdrop-blur-sm">
          <div className="w-full max-w-md bg-cyber-dark-card border border-cyber-cyan/30 rounded-xl shadow-2xl p-6">
            ...
            <p className="text-cyber-muted mb-6">
              确定要移除 <strong className="text-cyber-white">{employeeToRemove.roleSlug}</strong> 的智能体吗？
              此操作不可撤销。
            </p>
